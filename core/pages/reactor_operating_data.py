...
from time_series_data.reactor_operating_data import (
    get_points_between_dates,
    get_start_stop_intervals,
)
...
def reactor_operating_data():
    ...
    def plot_cards(
        ...
        with ui.row().classes("items-center"):
            ...
            ).classes("cursor-pointer ml-2 hover:bg-slate-200 rounded-full h-12 w-12")
        ...
        with ui.row():
            for reactor in ReactorOperatingData.objects.all():
                ...
                points: list[Point] = get_points_between_dates(
                    reactor, start_earliest_on_local_day, stop_latest_on_local_day
                )
                ...
                with ui.card():
                    with ui.row().classes("w-full"):
                        ...
                        ui.circular_progress(
                            round(points[-1].fields["pct"]), min=0, max=100, size="md"
                        ).classes("mr-2")
    ...
    with theme.frame():
        ...
        with ui.row():
            ...
            start_interval_date_str = datetime_converter.utc_to_local(
                start_interval
            ).strftime("%Y/%m/%d")
            stop_interval_date_str = datetime_converter.utc_to_local(
                stop_interval
            ).strftime("%Y/%m/%d")
            ...
            if today < start_interval:
                ...
                today = stop_interval
            today_interval_str_dashes = datetime_converter.utc_to_local(today).strftime(
                "%Y-%m-%d"
            )
